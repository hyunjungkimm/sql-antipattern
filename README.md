...
## 8. 다중 칼럼 속성
### 안티 패턴 : 여러 개의 컬럼 생성
- 값 검색 : 관련된 여러 개의 컬럼을 모두 확인해야 함
- 값 추가와 삭제 : 조회 후 업데이트를 해야함. 조회한 다음 업데이트 하기 전에 다른 클라이언트가 같은 행을 업데이트 하기 위해 동일한 절차를 실행하고 있다면 충돌이 발생할 수 있음.
- 동일한 값이 들어갈 수 있어 유일성 보장 못함
- 컬럼이 부족할 수 있음
### 안티 패턴 합당한 경우
- 속성이 각각 다른 경우
### 해법 : 종속 테이블 생성
- 다중 값 속성을 위한 칼럼을 하나 가지는 종속 테이블을 만드는 것

## 9. 메타데이터 트리블
- 트리블 : 스타트렉에 나온 애완동물로 처음에는 매력적으로 보이나, 번식력이 바퀴벌레급, 트리블 개체수를 관리하는 것이 문제
### 안티 패턴 : 테이블 또는 칼럼 복제 
- 엄청나게 많은 테이블이나 엄청나게 많은 컬럼을 만들어야 함
- 데이터 정합성 관리, 데이터 동기화, 유일성 보장, 여러 테이블에 걸쳐 조회하기, 메타데이터 동기화, 참조정합성 관리(테이블이 여러개면 fk에 하나의 테이블을 지정할 수 없음), 메타데이터 트리블 칼럼 식별하기
### 안티 패턴 합당한 경우
-  매일 사용하는 데이터와 오래된 데이터를 분리해 별도 보관하는 방식으로 테이블 수동 분할을 사용할 수 있음
### 해법 : 파티션과 정규화 
- 수평분할 : 행을 여러 파티션으로 분리하는 규칙과 함께 논리적 테이블을 하나 정의하면 물리적으로는 테이블이 분리되어 있지만, SQL에서는 마치 하나의 테이블인 것처럼 사용
<img width="353" alt="image" src="https://github.com/hyunjungkimm/sql-antipattern/assets/97015607/6a816809-371a-4588-932f-8a9886be0674">
    - MySQL 5.1에서 지원하는 파티셔닝 - 각 테이블에서 자신의 행을 별도 스토리지로 분리하는 방식
- 수직분할 : 칼럼으로 테이블을 나눔
- 메타데이터 트리블 칼럼을 종속 테이블로 만들기
